...
function projectFor(pkgDir, typesRoot) {
  ...
  // ---- FORK (chassis-react) --------------------------------------------
  // @chassis-ui/react declares no `types` / `typings` / `publishConfig.types`,
  // and that is correct for this package: tsdown emits dist/index.d.ts beside
  // dist/index.js, which TS resolves adjacently under both `bundler` and
  // node16-ESM (publint and `attw --profile esm-only` both pass clean).
  // Upstream then falls through to `<pkgDir>/index.d.ts`, which does not exist,
  // so getSourceFile(entry) is undefined and the entire export surface reads as
  // empty -> `exported PascalCase symbols: 0` -> 0 components discovered.
  // findTypesRoot() has already located the real .d.ts root by this point, so
  // prefer an index.d.ts inside it before falling back to the package-root guess.
  const declared = (pubEntry && existsSync(join(pkgDir, pubEntry)) ? pubEntry : null) || pj.types || pj.typings;
  const typesRootIndex = typesRoot ? join(typesRoot, 'index.d.ts') : null;
  const entry = declared
    ? join(pkgDir, declared)
    : typesRootIndex && existsSync(typesRootIndex)
      ? typesRootIndex
